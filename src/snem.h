...
static inline unsigned char readmem(unsigned long a)
{
	...
	//        if ((a>>12)==0x19E) printf("%06X %08X %08X
...
static inline void writemem(unsigned long ad, unsigned char v)
{
	...
	//        (ad&0xFFFF)==0x109 || (ad&0xFFFF)==0x108) printf ("WRITE %04X %02X
	//        %06X\n",ad,v,pbr|pc);
	//        if (((a>>16)&0x7F)<0x7E && (a&0x8000)) printf("Write %06X %02X
	//        %06X\n",a,v,pbr|pc);
	//        if (a>=0x1E0 && a<=0x2E0) printf("Write %06X %02X
	//        %06X\n",a,v,pbr|pc);
	//        if ((ad&0xFFFE)==0x7E) printf("Write %06X %02X %06X A
	//        %04X\n",ad,v,pbr|pc,a.w);
	//        if (ad<=0x7EAEFF && ad>=0x7EAC00) printf("Write %06X %02X %06X A
	//        %04X DBR %02X X %04X Y %04X\n",ad,v,pbr|pc,a.w,dbr,x.w,y.w);
	//        if (ad>=0x7E2500 && ad<=0x7E47E0) printf("Write %06X %02X %06X A
	//        %04X DBR %02X X %04X Y %04X\n",ad,v,pbr|pc,a.w,dbr,x.w,y.w);
	//        if (ad>=0x7E7368 && ad<=0x7E9468) printf("Write %06X %02X %06X A
	//        %04X DBR %02X X %04X Y %04X\n",ad,v,pbr|pc,a.w,dbr,x.w,y.w);
	//        if (a==0x93) printf("Write %06X %02X %06X\n",a,v,pbr|pc);
	//        if ((a&0xFFFFFF)==0x7E2568) printf("Write %06X %02X
	//        %06X\n",a,v,pbr|pc);
	//        if (a==0x1DDE || a==0x1DDF) printf("Write %06X %02X
	//        %06X\n",a,v,pbr|pc);
	//        if (a<0x7E8000 && a>=0x7E0000) printf("Write %06X %02X
	//        %06X\n",a,v,pbr|pc);
	//        if (a<0x7F8000 && a>=0x7F0D50) printf("Write %06X %02X
	//        %06X\n",a,v,pbr|pc);
