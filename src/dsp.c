...
void writedsp(uint16_t a, uint8_t v)
{
	...
	if (a & 1) {
		...
		dspregs[curdspreg] = v;
		// if (!(curdspreg&0xE)) { snemdebug("Write DSP %02X %02X
		// %04X\n",curdspreg,v,getspcpc()); }
		// if (curdspreg==4) { spcoutput=1; }
		// if (curdspreg==0x14) { spcoutput=0; }
		// if (v) { snemdebug("Write DSP %02X %02X
		// %04X\n",curdspreg,v,getspcpc()); }
		switch (curdspreg) {
		case 0x00:
		case 0x10:
		...
		case 0x76:
			...
			dsp.etype[curdspreg >> 4] =
				(dsp.gain[curdspreg >> 4] & 0x80) ? 1 : 0;
			dsp.etype[curdspreg >> 4] |=
		...
		case 0x77:
			...
			break;
		case 0x4C:
			// if (v) { snemdebug("Voice on %02X\n",v); }
			for (c = 0; c < 8; c++) {
				if (v & (1 << c)) {
					dsp.voiceon[c] = 1;
					dsp.voiceaddr[c] =
						spcram[(dsp.dir << 8) + dsp.sourcenum[c]] |
						(spcram[(dsp.dir << 8) + dsp.sourcenum[c] + 1] << 8);
					dsp.brrstat[c] = 0;
					dsp.pos[c] = 0;
...
inline int16_t decodebrr(int v, int c)
{
	...
	case 2:
		temp = temp + (lastsamp[c][0] << 1) +
			   ((-((lastsamp[c][0] << 1) + lastsamp[c][0])) >> 5) -
			   lastsamp[c][1] + (lastsamp[c][1] >> 4);
		break;
	case 3:
		temp = temp + (lastsamp[c][0] << 1) +
			   ((-(lastsamp[c][0] + (lastsamp[c][0] << 2) +
				   (lastsamp[c][0] << 3))) >>
				6) -
	...
	if (dsp.brrstat[c] & 1) { /* First nibble */
		...
		if (dsp.brrstat[c] == 17) {
			...
			if (dsp.brrctrl[c] & 1) {
				if (dsp.brrctrl[c] & 2) {
					dsp.voiceaddr[c] =
						spcram[(dsp.dir << 8) + dsp.sourcenum[c] + 2] |
						(spcram[(dsp.dir << 8) + dsp.sourcenum[c] + 3] << 8);
				} else {
					dsp.voiceon[c] = 0;
					dsp.endx |= (1 << c);
					// dsp.voiceend[c]=1;
...
void polldsp()
{
	...
	for (c = 0; c < 8; c++) {
		...
		// snemdebug("Overflow - %i %i %i\n",c,sample,totalsamplel); }
		// if (sample && dsp.evol[c]) { snemdebug("%i %04X %04X
		// ",c,sample,dsp.evol[c]); }
		sample *= dsp.evol[c];
		sample >>= 11;
		dsp.outx[c] = sample >> 8;
		// if (sample) { snemdebug("%04X %i %i
			...
			if (dsp.adsrstat[c] == RELEASE) {
				...
				case 3: /* ADSR */
					...
					case DECAY:
						...
						dsp.edelay[c] =
							ratetable[((dsp.adsr1[c] & 0x70) >> 2) | 1];
						break;
					case SUSTAIN:
						dsp.evol[c] -= ((dsp.evol[c] - 1) >> 8) + 1;
						// snemdebug("Evol
	...
	if (dsp.noisedelay <= 0) {
		...
		dsp.noise = (dsp.noise >> 1) |
					(((dsp.noise << 14) ^ (dsp.noise << 13)) & 0x4000);
	}
