...
/*                        pc7=pc8;
                        ...
                        pc3=pc4;
                        pc4=pbr|pc;
//                        if (pc==0xFB2A) printf("FB2A X=%04X\n",x.w);
                        if (pc==0xFD5F || pc==0xFD94)
                        {
                                printf("%04X %06X %06X %06X %06X %06X %06X %06X %06X %06X %06X %06X %06X\n",pc,pc2,pc3,pc4,pc7,pc8,pc9,pc10,pc11,pc12,pc13,pc14,pc15);
                                output=1;
                                timetolive=20;
                        }
                        if (pc==0xF58D) printf("F58D A=%04X Y=%04X\n",a.w,y.w);
...
//                        if (ins==6050000 && times==1) output=1;
                        if (oldnmi!=nmi && nmienable && nmi)
                           ...
                           irq65816();
                        oldnmi=nmi;
                        if (output) printf("%06X %02X A=%04X X=%04X Y=%04X S=%04X %02X %i %i %04X %i %02X %i %i\n",pc|pbr,dbr>>16,a.w,x.w,y.w,s.w,opcode,p.m,cpumode,dp,lines,lines,xirq,yirq);
                        if (timetolive)
                        {
                                timetolive--;
                                if (!timetolive)
...
//                        if (pc==0x9DCD) printf("99BA %04X %04X %04X %i\n",a.w,x.w,y.w,ins);
//                        if (pc==0xAA7C) printf("AA7C %04X %04X %04X %i\n",a.w,x.w,y.w,ins);
...
//                        snemlog("Enter VBL\n");
                ...
                if (lines==0xE3) joyscan=0;
                if (lines==200 && key[KEY_ESC]) break;
        }
        frames++;
}
