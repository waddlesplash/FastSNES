...
void resizewindow(int xsize, int ysize)
{
        ...
        MoveWindow(ghwnd,r.left,r.top,
                     ...
                     ysize+(GetSystemMetrics(SM_CYFIXEDFRAME)*2)+GetSystemMetrics(SM_CYMENUSIZE)+GetSystemMetrics(SM_CYCAPTION)+3,
                     TRUE);
}
...

{
        ...
        ghwnd=hwnd = CreateWindowEx (
           ...
           512+(GetSystemMetrics(SM_CXFIXEDFRAME)*2)+1,                 /* The programs width */
           448+(GetSystemMetrics(SM_CYFIXEDFRAME)*2)+GetSystemMetrics(SM_CYMENUSIZE)+GetSystemMetrics(SM_CYCAPTION)+2,                 /* and height in pixels */
...
/*                if (soundframe>=((pal)?5:6))
                ...
                if (!drawcount) WaitForSingleObject(frameobject,100);
...
//                   sleep(1);
                if (changefps)
                {
                        if (romloaded) sprintf(s,"NeuSneM v0.1 - %i fps",fps);
                        else           sprintf(s,"NeuSneM v0.1");
                        set_window_title(s);
                        changefps=0;
                }
//                sleep(0);
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
        ...
        switch (message)                  /* handle the messages */
        {
                ...
                switch (LOWORD(wParam))
                {
                        ...
                        return 0;
                        case IDM_FILE_LOAD:
                        if (!getfile(hwnd,"ROM image (*.SMC)\0*.SMC\0All files (*.*)\0*.*\0"))
                        {
                                romloaded=1;
