...
unsigned long zeropage() /*It's actually direct page, but I'm used to calling it zero page*/
{
...
unsigned long jindirectx() /*JSR (,x) uses PBR instead of DBR, and 2 byte address insted of 1 + dp*/
{
...
void ldaIndirectLongy16()
{
        addr=indirectly();
        a.w=readmemw(addr);
        if (pc==0xFDC9) printf("LDA %06X %04X\n",addr,a.w);
        setzn16(a.w);
}
