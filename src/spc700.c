...
void writespcregs(uint16_t a, uint8_t v)
{
	...
	case 0xF7:
		spctocpu[a & 3] = v;
		// snemdebug("SPC writes %02X to %02X\n",v,a);
		break;
	case 0xFA:
	case 0xFB:
	case 0xFC:
		spclimit[a - 0xFA] = v;
		// snemdebug("SPC limit %i = %02X\n",a-0xFA,v);
		break;
	case 0xFD:
	case 0xFE:
	case 0xFF:
...
#define readspc(a)                                                             \
	((((a)&0xFFC0) == 0xFFC0)                                                  \
		 ? spcreadhigh[(a)&63]                                                 \
		 : ((((a)&0xFFF0) == 0xF0) ? readspcregs(a) : spcram[a]))
#define writespc(a,                                                            \
				 v) /* if ((a)==0x17 || (a)==0x221) { snemdebug("Write %04X %02X     \
				 %04X\n",a,v,spc.pc); } */                                 \
	if (((a)&0xFFF0) == 0xF0)                                                  \
		writespcregs(a, v);                                                    \
	else                                                                       \
...
#define getdp()                                                                \
	...
	spc.pc++
#define getdpx()                                                               \
	addr = ((readspc(spc.pc) + spc.x) & 0xFF) | spc.p.p;                       \
	spc.pc++
#define getdpy()                                                               \
	addr = ((readspc(spc.pc) + spc.ya.b.y) & 0xFF) | spc.p.p;                  \
	spc.pc++
#define getabs()                                                               \
...
void dumpspcregs()
{
	...
	fclose(f);
	snemdebug("SPC registers :\n");
	snemdebug("YA=%04X X=%02X S=%02X PC=%04X\n", spc.ya.w, spc.x, spc.s, spc.pc);
	snemdebug("IN  : %02X %02X %02X %02X\n", ram[0xF4], ram[0xF5], ram[0xF6],
		   ram[0xF7]);
	snemdebug("OUT : %02X %02X %02X %02X\n", spctocpu[0], spctocpu[1], spctocpu[2],
		   spctocpu[3]);
}

#define ADC(ac, temp)                                                          \
	tempw = ac + temp + ((spc.p.c) ? 1 : 0);                                   \
...
void execspc()
{
	...
	while (spccycles > 0) {
		...
		case 0x16: /* OR A,(xxxx+Y) */
			addr = readspc(spc.pc) + (readspc(spc.pc + 1) << 8) + spc.ya.b.y;
			spc.pc += 2;
			spc.ya.b.a |= readspc(addr);
			setspczn(spc.ya.b.a);
			spccycles(5);
		...
		case 0x2D: /* PUSH A */
			writespc(spc.s + 0x100, spc.ya.b.a);
			spc.s--;
			spccycles(4);
			break;
		...
		case 0x6D: /* PUSH Y */
			writespc(spc.s + 0x100, spc.ya.b.y);
			spc.s--;
			spccycles(4);
			break;
		...
		case 0x7A: /* ADDW YA,xx */
			addr = readspc(spc.pc) + spc.p.p;
			spc.pc++;
			// snemdebug("ADDW %04X ",addr);
			tempw = readspc(addr) | (readspc(addr + 1) << 8);
			// snemdebug("%04X+%04X",tempw,spc.ya.w);
		...
		// spctotal+=spccount;
		// if (spcoutput) { snemdebug("%04X : %04X %02X %02X
		// %02X\n",spc.pc,spc.ya.w,spc.x,spc.s,opcode); }
		// if (spc.pc==0x12F7) { snemdebug("12F7 from %04X
		// %04X\n",spc2,spc3); }
	}
